    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float2 uv_MainTex : TEXCOORD0;  // UV coortinates for texture
            ...
            v2f vertexShader(float4 pos : POSITION0, float2 uv_MainTex : TEXCOORD0, float3 normal : NORMAL0)
            {
                ...
                pos = mul(UNITY_MATRIX_MVP, pos); // compute vertex position in screen space
                
                // get the distance between the camera and vertex
                o.depth = (1.0 - unity_OrthoParams.w) * 100 * pos.z +
